...
class RPMSystem:
    ...
    def _build_nuc_ops(self, d_spin, a_spin):
        Ix, Iy, Iz = qt.jmat(d_spin, 'x'), qt.jmat(d_spin, 'y'), qt.jmat(d_spin, 'z')
        ID = {
        ...
        if a_spin == 0:
            ...
            Ix, Iy, Iz = qt.jmat(a_spin, 'x'), qt.jmat(a_spin, 'y'), qt.jmat(a_spin, 'z')
            IA = {
...
def get_hyperfine_hamiltonian(sys: RPMSystem, Axx, Ayy, Azz, theta, phi):
    ...
    H_accum = sum(LAB[i, j] * sys.SD[d1] * sys.ID[d2] 
                  for i, d1 in enumerate(['x', 'y', 'z']) 
                  for j, d2 in enumerate(['x', 'y', 'z']) if LAB[i, j] != 0)
...
def get_hyperfine_hamiltonian_2nuc(sys: RPMSystem, Axx, Ayy, Azz, theta1, phi1, theta2, phi2, mult=1.0):
    PAX1 = GYRO_E * np.array([[Axx, 0, 0], [0, Ayy, 0], [0, 0, Azz]])
    R1 = get_rotation_matrix(theta1, phi1)
    LAB1 = R1 @ PAX1 @ R1.T 
    
    PAX2 = GYRO_E * np.array([[Axx * mult, 0, 0], [0, Ayy * mult, 0], [0, 0, Azz * mult]])
    R2 = get_rotation_matrix(theta2, phi2)
    LAB2 = R2 @ PAX2 @ R2.T
    ...
    for i, d1 in enumerate(dirs):
        for j, d2 in enumerate(dirs):
            if abs(LAB1[i, j]) > 1e-12:
                term1 = LAB1[i, j] * sys.SD[d1] * sys.ID[d2]
                H_accum = term1 if H_accum is None else H_accum + term1
            if abs(LAB2[i, j]) > 1e-12:
                term2 = LAB2[i, j] * sys.SA[d1] * sys.IA[d2]
                H_accum = term2 if H_accum is None else H_accum + term2
...
def get_dipolar_hamiltonian(sys: RPMSystem, Dxx, Dyy, Dzz, theta, phi):
    ...
    H_accum = sum(LAB[i, j] * sys.SD[d1] * sys.SA[d2] 
                  for i, d1 in enumerate(['x', 'y', 'z']) 
                  for j, d2 in enumerate(['x', 'y', 'z']) if LAB[i, j] != 0)
